import { AttendanceRecord, LeaveRequest, ESSProfile, UserChecklist, User, Role, WorkFromHomeRequest, Task, AppNotification } from '../types';
...
const NOTIFICATIONS_KEY = 'bytechsol_notifications';
...
const mapUserFromDb = (row: Record<string, any>): User => {
  ...
  return {
    ...
    homeAllowance: pickValue(row, ['home_allowance', 'homeAllowance'], undefined),
    travelAllowance: pickValue(row, ['travel_allowance', 'travelAllowance'], undefined),
    internetAllowance: pickValue(row, ['internet_allowance', 'internetAllowance'], undefined),
...
const mapUserToDb = (user: User) => ({
  ...
  home_allowance: user.homeAllowance ?? null,
  travel_allowance: user.travelAllowance ?? null,
  internet_allowance: user.internetAllowance ?? null,
...
const mapNotificationFromDb = (row: Record<string, any>): AppNotification => ({
  id: String(pickValue(row, ['id'], '')),
  userId: String(pickValue(row, ['user_id', 'userId'], '')),
  title: String(pickValue(row, ['title'], '')),
  message: String(pickValue(row, ['message'], '')),
  createdAt: String(pickValue(row, ['created_at', 'createdAt'], '')),
  read: Boolean(pickValue(row, ['read'], false)),
  autoGenerated: Boolean(pickValue(row, ['auto_generated', 'autoGenerated'], false)),
  playSound: Boolean(pickValue(row, ['play_sound', 'playSound'], false))
});

const mapNotificationToDb = (notification: AppNotification) => ({
  id: notification.id,
  ...
  auto_generated: notification.autoGenerated ?? false,
  play_sound: notification.playSound ?? false
});
...
export const fetchNotificationsRemote = async (): Promise<AppNotification[]> => {
  if (!isSupabaseConfigured || !supabase) {
    return loadLocal<AppNotification[]>(NOTIFICATIONS_KEY, []);
  }
  const { data, error } = await supabase.from('notifications').select('*');
  if (error || !data) {
    logSupabaseError('fetchNotificationsRemote', error);
    return loadLocal<AppNotification[]>(NOTIFICATIONS_KEY, []);
  }
  const mapped = data.map(mapNotificationFromDb);
  saveLocal(NOTIFICATIONS_KEY, mapped);
  return mapped;
};

export const saveNotifications = async (notifications: AppNotification[]) => {
  saveLocal(NOTIFICATIONS_KEY, notifications);
  if (!isSupabaseConfigured || !supabase) return;
  const payload = notifications.map(mapNotificationToDb);
  const { error } = await supabase.from('notifications').upsert(payload, { onConflict: 'id' });
  if (error) {
    logSupabaseError('saveNotifications', error);
  }
};
